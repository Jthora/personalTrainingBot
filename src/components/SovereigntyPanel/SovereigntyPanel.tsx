...
import { restartStoreSyncs } from '../../services/gunStoreSyncs';
import { trackEvent } from '../../utils/telemetry';
import type { SyncStatus } from '../../services/syncStatusStore';

const QRCodeDisplay = lazy(() => import('../QRCodeDisplay/QRCodeDisplay'));
const QRCodeScanner = lazy(() => import('../QRCodeScanner/QRCodeScanner'));
...
/** Max bytes for QR code at error correction level H, version 40 */
const QR_MAX_BYTES = 1273;
...
const SovereigntyPanel: React.FC = () => {
  ...
  const handleGenerateQr = useCallback(async () => {
    ...
    try {
      ...
      if (byteLength > QR_MAX_BYTES) {
        setExportError(
          `Export is ${byteLength} bytes — exceeds QR capacity (${QR_MAX_BYTES} bytes at error correction H). ` +
          `${exportPassphrase ? 'Try without a passphrase, or use' : 'Use'} the Download tab instead.`
        );
        return;
      }
      setQrJson(json);
      trackEvent({ category: 'p2p', action: 'gun_qr_export_generate', source: 'ui' });
    } catch (err) {
      setExportError(err instanceof Error ? err.message : 'Export failed');
    } finally {
      setQrWorking(false);
  ...
  const handleQrScan = useCallback(async (decoded: string) => {
    ...
    try {
      ...
      // Export envelope is { encrypted: boolean, data: identity | SEA-encrypted-string }
      const isEncryptedEnvelope = parsed?.encrypted === true;
      const isPlainEnvelope = parsed?.encrypted === false && parsed?.data?.keypair?.pub;
      if (!isEncryptedEnvelope && !isPlainEnvelope) {
        setImportError('QR code not recognized as a keypair export');
        return;
      }
    } catch {
      setImportError('QR code not recognized as a keypair export');
    ...
    try {
      ...
      restartStoreSyncs();
      trackEvent({ category: 'p2p', action: 'gun_qr_scan_import', source: 'ui' });
      setOverlayMode(null);
      setImportText('');
      setImportPassphrase('');
    } catch (err) {
              ...
              {!qrJson ? (
                ...
                <>
                  ...
                  <div className={styles.qrWrapper}>
                    <Suspense fallback={<p className={styles.overlayDesc}>Loading…</p>}>
                      <QRCodeDisplay
                        value={qrJson}
                        size={240}
        ...
        >
          ...
          {importTab === 'scan' && (
            <div className={styles.scanBlock}>
              ...
              <label className={styles.fieldLabel} htmlFor="scan-passphrase">
                Passphrase (if encrypted)
              </label>
              <input
                id="scan-passphrase"
                type="password"
                className={styles.textInput}
                placeholder="Leave blank if QR was unencrypted"
                value={importPassphrase}
                onChange={(e) => setImportPassphrase(e.target.value)}
                data-testid="sovereignty-scan-passphrase"
              />

              <Suspense fallback={<p className={styles.overlayDesc}>Loading scanner…</p>}>
                <QRCodeScanner
                  onScan={handleQrScan}
