...
const createModernMissionScheduleSync = (): MissionSchedule => {
    const drillCount = 10;
    const date = new Date().toISOString().split('T')[0];

    const cache = DrillCategoryCache.getInstance();
    const drills = cache.getAllDrillsSelected();
    ...
    const selectedDrills = getArchetypeWeightedItems(filteredDrills, Math.min(drillCount, filteredDrills.length));

    const missionSets: MissionSet[] = [];
    for (let i = 0; i < selectedDrills.length; i += 3) {
        const drillsSlice = selectedDrills.slice(i, i + 3);
        const drillsWithCompletion = drillsSlice.map(drill => [drill, false] as [Drill, boolean]);
        missionSets.push(new MissionSet(drillsWithCompletion));
    }
...
const createModernMissionSchedule = async (): Promise<MissionSchedule> => {
    ...
    console.log('Filtered drills based on difficulty level:', filteredDrills.length);
    ...
    for (let i = 0; i < selectedDrills.length; i += 3) {
        const drillsSlice = selectedDrills.slice(i, i + 3);
        const drillsWithCompletion = drillsSlice.map(drill => [drill, false] as [Drill, boolean]);
        const missionSet = new MissionSet(drillsWithCompletion);
...
export const createMissionSchedule = async (): Promise<MissionSchedule> => {
    return createModernMissionSchedule();
};

export const createMissionScheduleSync = (): MissionSchedule => {
    return createModernMissionScheduleSync();
};